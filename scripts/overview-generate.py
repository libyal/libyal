...
class ProjectsReader:
    ...
    def ReadFromFile(self, filename):
        ...
        for project_name in self._config_parser.sections():
            ...
            project.display_name = self._GetConfigValue(project_name, "display_name")
...
class OverviewWikiPageGenerator(WikiPageGenerator):
    ...
    def Generate(self, projects, output_writer):
        ...
        for category in self._ORDER_OF_OTHER_CATEGORIES:
            template_mappings = {"category_title": self._CATEGORIES[category][0]}
...
class StatusWikiPageGenerator(WikiPageGenerator):
    ...
    def Generate(self, projects, output_writer):
        ...
        for configuration, projects_per_version in sorted(
            ...
            self._GenerateSection("table_header.txt", template_mappings, output_writer)
        ...
        for script, projects_per_version in sorted(versions_per_script.items()):
            template_mappings = {
                "title": script,
            }
            self._GenerateSection("table_header.txt", template_mappings, output_writer)
        ...
        for m4_script, projects_per_version in sorted(versions_per_m4_script.items()):
            template_mappings = {
                "title": m4_script,
            }
            self._GenerateSection("table_header.txt", template_mappings, output_writer)
        ...
        for category in self._ORDER_OF_LIBRARY_CATEGORIES:
            ...
            for library, projects_per_version in sorted(versions_per_library.items()):
                template_mappings = {
                    "title": library,
                }
                self._GenerateSection(
        ...
        for script, projects_per_version in sorted(versions_per_test_script.items()):
            template_mappings = {
                "title": script,
            }
            self._GenerateSection("table_header.txt", template_mappings, output_writer)
