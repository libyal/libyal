...
void (*${tools_name}_signal_signal_handler)( ${tools_name}_signal_t ) = NULL;
...
{
	static char *function = "${tools_name}_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( ${tools_name}_signal_signal_handler != NULL )
			{
				${tools_name}_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "${tools_name}_signal_attach";
	...
	${tools_name}_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) ${tools_name}_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	${tools_name}_signal_initialize_memory_debug();
#endif
...
int ${tools_name}_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "${tools_name}_signal_detach";
	...
	${tools_name}_signal_signal_handler = NULL;
