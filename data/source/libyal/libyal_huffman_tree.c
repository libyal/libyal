...
{
	static char *function = "${library_name}_huffman_tree_initialize";
	size_t array_size     = 0;
	...
	*huffman_tree = memory_allocate_structure(
	...
	if( memory_set(
	     *huffman_tree,
	     0,
	     sizeof( ${library_name}_huffman_tree_t ) ) == NULL )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_MEMORY,
...
int ${library_name}_huffman_tree_free(
     ...
     libcerror_error_t **error )
{
	static char *function = "${library_name}_huffman_tree_free";
     ...
     libcerror_error_t **error )
{
	...
	static char *function = "${library_name}_huffman_tree_build";
	size_t array_size     = 0;
	uint16_t symbol       = 0;
	uint8_t bit_index     = 0;
	uint8_t code_size     = 0;
...
int ${library_name}_huffman_tree_get_symbol_from_bit_stream(
     ...
     libcerror_error_t **error )
{
	static char *function  = "${library_name}_huffman_tree_get_symbol_from_bit_stream";
