...
/* Define to make ${library_name_suffix}_test_read generate verbose output
#define ${library_name_suffix_upper_case}_TEST_READ_VERBOSE
 */

#define ${library_name_suffix_upper_case}_TEST_READ_BUFFER_SIZE		4096
#define ${library_name_suffix_upper_case}_TEST_READ_NUMBER_OF_THREADS	4
...
/* Tests ${library_name}_file_read_buffer
 * Returns 1 if successful, 0 if not or -1 on error
 */
int ${library_name_suffix}_test_read_buffer(
     ${library_name}_file_t *file,
     size64_t input_size,
     size64_t expected_size )
{
	uint8_t buffer[ ${library_name_suffix_upper_case}_TEST_READ_BUFFER_SIZE ];
	...
	{
		read_size = ${library_name_suffix_upper_case}_TEST_READ_BUFFER_SIZE;
...
/* Tests ${library_name}_file_read_buffer_at_offset
 * Returns 1 if successful, 0 if not or -1 on error
 */
int ${library_name_suffix}_test_read_buffer_at_offset(
     ${library_name}_file_t *file,
     off64_t input_offset,
     size64_t input_size,
     off64_t expected_offset,
     size64_t expected_size )
{
	...
	if( ${library_name}_file_get_offset(
	     file,
	     &result_offset,
	     &error ) != 1 )
	{
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int ${library_name_suffix}_test_seek_offset_and_read_buffer(
     ${library_name}_file_t *file,
     off64_t input_offset,
     int input_whence,
     size64_t input_size,
     ...
     size64_t expected_size )
{
	...
	if( result == 1 )
	{
		if( input_offset >= 0 )
		{
			result = ${library_name_suffix}_test_read_buffer(
				  file,
				  input_size,
				  expected_size );
		}
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int ${library_name_suffix}_test_read_from_file(
     ${library_name}_file_t *file,
     size64_t file_size )
{
	...
	result = ${library_name_suffix}_test_seek_offset_and_read_buffer(
	          file,
	          read_offset,
	          SEEK_SET,
	          read_size,
	...
	result = ${library_name_suffix}_test_read_buffer_at_offset(
	          file,
	          read_offset,
	          read_size,
     ...
     libcerror_error_t **error )
{
	...
	result = ${library_name_suffix}_test_read_from_file(
	          file,
	          file_size );
 ...
 */
int ${library_name_suffix}_test_read_callback_function(
     ${library_name}_file_t *file,
     void *arguments ${library_name_suffix_upper_case}_TEST_ATTRIBUTE_UNUSED )
{
	uint8_t buffer[ ${library_name_suffix_upper_case}_TEST_READ_BUFFER_SIZE ];

	libcerror_error_t *error = NULL;
	static char *function    = "${library_name_suffix}_test_read_callback_function";
	...
	while( number_of_iterations > 0 )
	{
		...
		{
			if( ${library_name}_file_seek_offset(
			     file,
			     (off64_t) -read_size,
			     SEEK_CUR,
			     &error ) == -1 )
     ...
     int number_of_threads )
{
	...
	static char *function                  = "${library_name_suffix}_test_read_from_file_multi_thread";
	...
	if( number_of_threads > 1 )
	{
		...
		if( libcthreads_thread_pool_create(
		     ...
		     number_of_threads,
		     number_of_iterations,
		     (int (*)(intptr_t *, void *)) &${library_name_suffix}_test_read_callback_function,
		     NULL,
		     &error ) != 1 )
		{
			libcerror_error_set(
...
#endif
{
...
#if defined( HAVE_DEBUG_OUTPUT ) && defined( ${library_name_suffix_upper_case}_TEST_READ_VERBOSE )
	...
	${library_name}_notify_set_stream(
	 ...
	 NULL );
#endif
	if( ${library_name_suffix}_test_read(
	     source,
	     &error ) != 1 )
	{
		fprintf(
...
#if defined( HAVE_MULTI_THREAD_SUPPORT )
	if( ${library_name_suffix}_test_read_multi_thread(
	     source,
	     &error ) != 1 )
	{
		fprintf(
