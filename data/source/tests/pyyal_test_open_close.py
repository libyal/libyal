...
def ${python_module_name}_test_single_open_close_file(filename, mode):
  if not filename:
...
def ${python_module_name}_test_multi_open_close_file(filename, mode):
...
def ${python_module_name}_test_single_open_close_file_object(filename, mode):
...
def ${python_module_name}_test_single_open_close_file_object_with_dereference(
    filename, mode):
...
def ${python_module_name}_test_multi_open_close_file_object(filename, mode):
...
def main():
  ...
  if not ${python_module_name}_test_single_open_close_file(options.source, "r"):
    return False

  if not ${python_module_name}_test_single_open_close_file(None, "r"):
    return False

  if not ${python_module_name}_test_single_open_close_file(options.source, "w"):
    return False

  if not ${python_module_name}_test_multi_open_close_file(options.source, "r"):
    return False

  if not ${python_module_name}_test_single_open_close_file_object(options.source, "r"):
    return False

  if not ${python_module_name}_test_single_open_close_file_object_with_dereference(
      options.source, "r"):
    return False

  if not ${python_module_name}_test_multi_open_close_file_object(
      options.source, "r"):
    return False
