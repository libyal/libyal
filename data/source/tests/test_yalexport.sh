...
OPTIONS_PER_PROFILE=(${tests_export_tool_options_per_profile_sh});
OPTION_SETS=(${tests_export_tool_option_sets_sh});
...
do
	...
	do
		...
		do
			...
			then
				run_test_on_input_file "$${TEST_SET_DIRECTORY}" "${library_name_suffix}export" "with_stdout_reference" "" "$${TEST_EXECUTABLE}" "$${INPUT_FILE}" "$${PROFILE_OPTIONS[@]}";
				RESULT=$$?;
			fi
