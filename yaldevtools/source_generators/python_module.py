...
class PythonModuleSourceFileGenerator(interface.SourceFileGenerator):
    ...
    def _GenerateTypeHeaderFile(
        ...
        if (
            ...
            and type_name == "security_identifier"
        ):
            with_parent = True
        else:
            ...
            if not os.path.exists(template_filename):
                ...
                if type_function == "set_key":
                    ...
                    if python_function_prototype.data_type == (
                        ...
                        if (
                            has_pseudo_sub_types
                            and python_function_prototype.value_type
                            not in value_type_objects
                        ):
    ...
    def _GenerateTypeSourceFileTypeObjectMethods(
        ...
        for type_function, python_function_prototype in iter(
            ...
            if (
                ...
                and "read_buffer" in python_function_prototypes
                and "seek_offset" in python_function_prototypes
            ):

                read_buffer_description = python_function_prototypes[
                    "read_buffer"
                ].GetDescription()
                seek_offset_description = python_function_prototypes[
                    "seek_offset"
                ].GetDescription()
                ...
                python_function_prototype.DataTypeIsDatetime()
                and type_function != "get_data_as_datetime"
            ):
                python_type_object_methods.extend(
                    [
                        "",
                ...
                and python_function_prototype.data_type
                in (definitions.DATA_TYPE_OBJECT, definitions.DATA_TYPE_STRING)
            ):
    ...
    def _GenerateTypeSourceFileTypeObjectGetSetDefinitions(
        ...
        for type_function, python_function_prototype in iter(
            ...
            if type_function.endswith("_by_name") or type_function.endswith("_by_path"):
                continue
        ...
        python_type_object_get_set_definitions.extend(
            ["", "\t/* Sentinel */", "\t{ NULL, NULL, NULL, NULL, NULL }"]
        )
    ...
    def _GetPythonTypeObjectFunctionPrototype(
        ...
        if (
            type_function == "get_cache_directory_name"
            and project_configuration.library_name == "libmsiecf"
        ):
            ...
            if (
                value_argument_string == "const uint8_t *byte_stream"
                and value_size_argument_string == "size_t byte_stream_size"
            ):
                data_type = definitions.DATA_TYPE_BINARY_DATA

            elif (
                value_argument_string == "const uint8_t *data"
                and value_size_argument_string == "size_t data_size"
            ):
                data_type = definitions.DATA_TYPE_BINARY_DATA
            ...
            if type_function == "get_ascii_codepage":
                ...
                if value_argument_string.startswith(
                    "int "
                ) and value_argument_suffix in ("entry", "index"):
                    function_type = definitions.FUNCTION_TYPE_GET_BY_INDEX
                ...
                if value_argument_index + 1 < len(function_prototype.arguments):
                    ...
                    if value_argument_string == "uint64_t *filetime":
                        ...
                        value_argument_string == "uint8_t *data"
                        and value_size_argument_string == "size_t data_size"
                    ):
                        data_type = definitions.DATA_TYPE_BINARY_DATA

                    elif (
                        value_argument_string == "uint8_t *guid_data"
                        and value_size_argument_string == "size_t guid_data_size"
                    ):
                        data_type = definitions.DATA_TYPE_GUID

                    elif (
                        value_argument_string == "uint8_t *utf8_string"
                        and value_size_argument_string == "size_t utf8_string_size"
                    ):
                        data_type = definitions.DATA_TYPE_STRING

                    elif (
                        value_argument_string == "uint8_t *uuid_data"
                        and value_size_argument_string == "size_t uuid_data_size"
                    ):
                        data_type = definitions.DATA_TYPE_UUID

                    elif (
                        value_argument_string == "char *string"
                        and value_size_argument_string == "size_t string_size"
                    ):
                        data_type = definitions.DATA_TYPE_NARROW_STRING
        ...
        if function_prototype.return_values:
            return_values = set()
            if (
                "0" in function_prototype.return_values
                or "NULL" in function_prototype.return_values
            ):
                return_values.add("None")

        python_function_prototype = source_code.PythonTypeObjectFunctionPrototype(
            project_configuration.python_module_name, type_name, python_type_function
        )
    ...
    def _GetPythonTypeObjectFunctionPrototypes(
        ...
        for function_name, function_prototype in iter(functions_per_name.items()):
            ...
            if type_function.startswith("get_") and type_function.endswith(
                "_data_size"
            ):
                continue

            # Skip functions that retrieve the size of an UTF-8 string.
            if type_function.startswith("get_utf8_") and type_function.endswith(
                "_size"
            ):
                continue
            ...
            if (
                ...
                or type_function.startswith("set_utf16_")
                or (
                    type_function.startswith("get_")
                    and (
                        type_function.endswith("_by_utf16_name")
                        or type_function.endswith("_by_utf16_path")
                        or type_function.endswith("_utf16_string")
                        or type_function.endswith("_utf16_string_size")
                    )
                )
            ):
                continue
            ...
            if (
                type_function == "get_flags"
                and project_configuration.library_name not in ("libfwnt",)
            ):
                continue

            # TODO: improve check only to apply for types with pseudo types.
            if type_function == "get_type" and project_configuration.library_name in (
                "libmsiecf",
                "libolecf",
                "libpff",
            ):
            ...
            if (
                type_function.startswith("get_value_")
                and type_function != "get_value_type"
                and project_configuration.library_name in ("libolecf",)
            ):
                continue

            if (
                type_function == "get_version"
                and project_configuration.library_name in ("libevt", "libevtx")
            ):
                continue

            if type_function.startswith(
                "write_buffer"
            ) and project_configuration.library_name not in ("libewf",):
                continue

            if type_function in (
                "get_offset_range",
                "get_number_of_unallocated_blocks",
                ...
                for type_function, python_function_prototype in iter(
                    ...
                    if sequence_type_name:
                        types_with_sequence_types.add(
                            (sequence_type_name, type_is_object)
                        )
