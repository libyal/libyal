...
class ConfigurationFileGenerator(interface.SourceFileGenerator):
    ...
    def _GenerateConfigureAC(self, project_configuration, template_mappings):
        ...
        for dependency in library_dependencies:
            ...
            if dependency.name == "libcaes":
                if project_configuration.library_name in ("libbde", "libluksde"):
                    ...
                    build_options.append(("AES-CBC support", "$ac_cv_libcaes_aes_cbc"))
        ...
        if (
            ...
            or "libcthreads" in makefile_am_file.libraries
        ):
        ...
        if (
            include_header_file
            and include_header_file.have_wide_character_type
            or project_configuration.HasTools()
        ):
    ...
    def _GenerateDpkg(
        self, project_configuration, template_mappings, output_writer, output_directory
    ):
        ...
        for directory_entry in os.listdir(templates_path):
            ...
            if directory_entry.startswith("control") or directory_entry.startswith(
                "rules"
            ):
                continue
            ...
            if directory_entry.endswith(".install"):
                if (
                    ...
                    and "-python" in directory_entry
                ):
                    continue

                if not project_configuration.HasTools() and "-tools" in directory_entry:
                    continue
        ...
        if project_configuration.HasPythonModule() and project_configuration.HasTools():
    ...
    def _GenerateGitignore(self, project_configuration, template_mappings):
        ...
        for source_file_path in sorted(glob.glob(source_glob)):
            if (
                source_file_path.endswith("_functions.c")
                or source_file_path.endswith("_getopt.c")
                or source_file_path.endswith("_i18n.c")
                or source_file_path.endswith("_memory.c")
                or source_file_path.endswith("_rwlock.c")
            ):
                continue
    ...
    def _GenerateRpmSpec(self, project_configuration, template_mappings):
        ...
        template_mappings["tools_spec_build_requires"] = " ".join(
            tools_spec_build_requires
        )
    ...
    def Generate(self, project_configuration, output_writer):
        ...
        for directory_entry in os.listdir(self._templates_path):
            ...
            if directory_entry in (
                ...
                "gitignore.yaml",
                "libyal.spec.in.yaml",
                "pyproject.toml.in.yaml",
                "tox.ini.yaml",
            ):
                continue

            if (
                directory_entry == "_build.py"
                and not project_configuration.HasPythonModule()
            ):
                continue
